...
import { drawNebuluxeWordmark, WORDMARK_WIDTH, WORDMARK_HEIGHT } from '../brand/nebuluxeWordmark';
...
const TextReveal = forwardRef((_, ref) => {
  ...
  const {
    ...
    const subtitle = 'A universe waiting to unfold';
    const subtitleColor = '#eeba7b';
    const pixelRatio = gl.getPixelRatio();
    ...
    const nameWidth = Math.min(460 * pixelRatio, maxContentWidth);
    const wordmarkHeight = nameWidth * WORDMARK_HEIGHT / WORDMARK_WIDTH;
    const nameHeight = Math.ceil(wordmarkHeight * 1.2);
    ...
    drawNebuluxeWordmark(ctx, (width - nameWidth) / 2, (nameHeight - wordmarkHeight) / 2, nameWidth);
  ...
  }, [size.width, subtitleFontFamily, gl]);
...
export default function ShootingStarIntro({ orbitProgress, storyEnd = SUBTITLE_STORY_END, rendering = true }) {
  ...
  return (
    <div ref={overlayRef} className={styles.overlay}>
      <Canvas
        frameloop={rendering && isVisible ? 'always' : 'never'}
        camera={{
