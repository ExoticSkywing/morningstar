...
const SECTIONS = [
  ...
  { key: 'projects', label: 'Projects', href: 'https://behfar.dev/' },
  { key: 'contact', label: 'Contact Me', href: 'mailto:behfar.behzad@gmail.com' },
];

export default function SceneNav({ activeSection, onNavigate, visible = true, sections = SECTIONS }) {
  ...
  const items = (className) =>
    sections.map(({ key, label, href, icon, wordmark }) => {
      const Item = href ? 'a' : 'button';
      return (
        <Item
          key={key}
          ...
          onClick={() => (href ? setIsOpen(false) : handleSelect(key))}
        >
          {icon && <img className={styles.brandMark} src={icon} alt="" width="24" height="24" />}
          {wordmark ? <img className={styles.brandWordmark} src={wordmark} alt={label} width="418" height="48" /> : label}
        </Item>
      );
    });
