...
export default function Universe({
  ...
  useEffect(() => {
    ...
    let inView = true;
    const shouldRender = () => renderingRef.current && inView && !document.hidden;
    ...
    const syncLoop = () => {
      if (shouldRender() && animFrame === null) {
        timer.reset();
        animFrame = window.requestAnimationFrame(tick);
      } else if (!shouldRender() && animFrame !== null) {
  ...
  return (
    <div
      ...
      style={{
        height: embedded ? '100svh' : `${HERO_HEIGHT_VH}vh`,
    ...
    >
      <div className={styles.sticky}>
        ...
        <div style={{ opacity: plungeTextOpacity }}>
          {showOverlays && (
            <ShootingStarIntro orbitProgress={orbitProgress} storyEnd={SUBTITLE_STORY_END} rendering={rendering} />
          )}

          {showOverlays && (
            <CaptionGravity orbitProgress={captionProgress} captions={ORBIT_CAPTIONS} rendering={rendering} />
          )}
        </div>
